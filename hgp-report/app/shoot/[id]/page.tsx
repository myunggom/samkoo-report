...
import ReportEditor from "@/components/shoot/ReportEditor";
import EmailComposer from "@/components/shoot/EmailComposer";
import ScheduleModal from "@/components/shoot/ScheduleModal";
import type { Report, Schedule } from "@/lib/shoot/types";
import { prettyDateTime } from "@/lib/shoot/format";
...
export default function ShootPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    (async () => {
      const [sRes, rRes] = await Promise.all([
        fetch(`/api/shoot/schedules/${id}`, { cache: "no-store" }),
        fetch(`/api/shoot/reports/${id}`, { cache: "no-store" }),
      ]);
      if (!sRes.ok) {
        setNotFound(true);
        setLoading(false);
  ...
  async function remove() {
    ...
    await fetch(`/api/shoot/schedules/${id}`, { method: "DELETE" });
    router.push("/shoot");
  }
  ...
  if (notFound || !schedule || !report)
    return (
      <div className="space-y-3">
        ...
        <Link href="/shoot" className="text-sm font-medium text-slate-900 underline">
          ← 촬영 스케줄로
        </Link>
      </div>
    );
  ...
  return (
    <div className="space-y-5">
      <Link href="/shoot" className="inline-block text-sm text-slate-500 hover:text-slate-800">
        ← 촬영 스케줄
      </Link>
