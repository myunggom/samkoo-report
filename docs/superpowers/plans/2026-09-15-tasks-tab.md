...
npm run build               # 아래 이유로 최초 1회 반드시 먼저
git branch --show-current   # feat/tasks-tab 여야 한다
```

> **`npx tsc --noEmit`을 처음 돌리기 전에 `npm run build`를 한 번 해야 한다.**
> `app/layout.tsx`가 쓰는 `LayoutProps`는 Next가 빌드 때 `.next/types`에 만들어 주는 전역 타입이라,
> 갓 clone한 상태에서 `tsc`만 돌리면 `Cannot find name 'LayoutProps'`가 난다. 이 작업과 무관한 기존 조건이다.
>
> 또한 `tsconfig.json`에 **`"allowImportingTsExtensions": true`** 를 추가해야 한다
> (`lib/tasks.check.mts`가 `./tasks.ts`를 확장자까지 명시해 import하는데, Node 실행에는 그 확장자가 필수다).
> `noEmit: true`가 이미 켜져 있어 전제조건을 만족하며 런타임 동작은 바뀌지 않는다.
